...
bool bijson_object_get_index(const bijson_t *bijson, size_t index, bijson_t *key_result, bijson_t *value_result) {
    ...
    size_t count = count + SIZE_C(1);

    const uint8_t *key_index = buffer = count_end;
    ...
    buffer += count * key_index_item_size;
    const uint8_t *value_index = buffer;

    buffer += count_1 * value_index_item_size;
    const uint8_t *key_data_start = buffer;


    uint64_t raw_last_key_end_offset =
        _bijson_read_minimal_int(key_data_start + key_index_item_size * count_1, key_index_item_size);
    ...
    const uint8_t *value_data_start = buffer + last_key_end_offset;
...
bool bijson_array_get_index(const bijson_t *bijson, size_t index, bijson_t *result) {
    ...
    buffer += (count - SIZE_C(1)) * index_item_size;
    ...
    uint64_t raw_start_offset = index
        ? _bijson_read_minimal_int(buffer + index_item_size * (index - SIZE_C(1)), index_item_size)
        : UINT64_C(0);
    ...
    uint64_t raw_end_offset = index == count - SIZE_C(1)
        ? data_size
        : _bijson_read_minimal_int(buffer + index_item_size * index, index_item_size);
    ...
    if(result) {
        result->buffer = buffer + start_offset;
